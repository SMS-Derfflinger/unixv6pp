...
#[no_mangle]
pub unsafe extern "C" fn prd_set_base_address(
    prd: *mut PhysicalRegionDescriptor,
    phy_base_addr: u32,
) {
    update_prd_unaligned(prd, |prd| prd.set_base_address(phy_base_addr));
}

#[no_mangle]
pub unsafe extern "C" fn prd_set_byte_count(prd: *mut PhysicalRegionDescriptor, bytes: u16) {
    update_prd_unaligned(prd, |prd| prd.set_byte_count(bytes));
}

#[no_mangle]
pub unsafe extern "C" fn prd_set_end_of_table(prd: *mut PhysicalRegionDescriptor, eot: bool) {
    update_prd_unaligned(prd, |prd| prd.set_end_of_table(eot));
}

#[no_mangle]
pub unsafe extern "C" fn prd_table_set_physical_region_descriptor(
    table: *mut PRDTable,
    index: i32,
    prd: *const PhysicalRegionDescriptor,
    eot: bool,
) {
    ...
    if index < 0 || index as usize >= PRDTable::NSIZE || prd.is_null() {
        return;
    }

    let prd = core::ptr::read_unaligned(prd);
    table.set_physical_region_descriptor(index as usize, prd, eot);
}

#[no_mangle]
pub unsafe extern "C" fn prd_table_base_address(table: *const PRDTable) -> u32 {
...
unsafe fn update_prd_unaligned<F>(prd: *mut PhysicalRegionDescriptor, update: F)
where
    F: FnOnce(&mut PhysicalRegionDescriptor),
{
    if prd.is_null() {
        return;
    }

    let mut value = core::ptr::read_unaligned(prd);
    update(&mut value);
...
#[no_mangle]
pub extern "C" fn dma_reset() {
    DMA::reset();
}

#[no_mangle]
pub extern "C" fn dma_is_error() -> bool {
    DMA::is_error()
}

#[no_mangle]
pub extern "C" fn dma_start(dma_type: i32, base_address: u32) {
    if dma_type == 0 {
        DMA::start(DMAType::Write, base_address);
    } else {
        DMA::start(DMAType::Read, base_address);
    }
}
