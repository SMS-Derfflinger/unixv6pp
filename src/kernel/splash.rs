...
use crate::{
    interrupt::{get_time, sleep_kernel_until},
...
const SPLASH_BMP: &'static CStr =
    unsafe { CStr::from_bytes_with_nul_unchecked(b"/v6pp_splash.bmp\0") };
...
pub fn splash() {
    ...
    sleep_kernel_until(get_time() + 1, -1);
    vesa_clear(0);
}

fn draw_img(file_path: &CStr) {
    ...
    if file_header.magic != [b'B', b'M'] {
        crate::println_warn!("splash: bad magic!");
        return;
    }
